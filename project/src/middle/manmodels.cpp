...
void ManModels::initObjectsTable() {
    // 设置标题内容
    QStringList labels;
    labels << QString("对象名称") << QString("大小") << QString("更新时间");
    m_modelObjects->setColumnCount(labels.size());
    m_modelObjects->setHorizontalHeaderLabels(labels);
}

QStandardItemModel* ManModels::modelBuckets() const { return m_modelBuckets; }

QStandardItemModel* ManModels::modelObjects() const { return m_modelObjects; }
