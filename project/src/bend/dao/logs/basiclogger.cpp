...
*/
BasicLogger::BasicLogger(QObject *parent) : QObject{parent} {
    m_td = new QThread();
    moveToThread(m_td);
    m_td->start();
}
...
void BasicLogger::onLog(const QString &file, int line, const QString &func, void *tid, int level, const QVariant &var,
                        bool up) {
