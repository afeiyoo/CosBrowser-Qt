...
void UiBreadWidget::setPath(const QString &path) { setPath(path.split("/", Qt::SkipEmptyParts)); }
...
void UiBreadWidget::clear() { m_model->clear(); }

QString UiBreadWidget::currentPath() const { return getPath(); }

QString UiBreadWidget::rootName() const { return m_rootName; }

bool UiBreadWidget::isEmpty() const { return ui->listView->model()->rowCount() == 0; }

bool UiBreadWidget::isRoot() const { return getPath() == m_rootName; }
...
QStandardItem *UiBreadWidget::currentItem() const { return m_model->item(m_model->rowCount() - 1); }
