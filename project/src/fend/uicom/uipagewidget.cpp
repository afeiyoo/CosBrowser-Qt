...
int UiPageWidget::totalRow() const { return m_totalRow; }

int UiPageWidget::maxRow() const { return m_maxRow; }

int UiPageWidget::currentPage() const { return m_currentPage; }
...
int UiPageWidget::isFirstPage() const { return currentPage() == 1; }

int UiPageWidget::isLastPage() const { return currentPage() == pageCount(); }
...
void UiPageWidget::next() { jumpTo(currentPage() + 1); }

void UiPageWidget::pre() { jumpTo(currentPage() - 1); }
...
void UiPageWidget::onCurrentPageSelected(const QString &text) {
    if (m_currentPage != text.toInt()) jumpTo(text.toInt());
}

void UiPageWidget::onMaxRowsItemSelected(const QString &text) { setMaxRow(text.toInt()); }
